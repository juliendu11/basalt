...
> ⚠️ Cette dédup joue contre les jobs de **tous** les états, y compris les jobs `completed`/`failed` **retenus** par `removeOnComplete`/`removeOnFail` (que BullMQ ne purge que paresseusement, à la complétion d'un autre job de la même file — pas sur une minuterie). Un `jobId` réutilisé dont la portée dépasse la fenêtre de rétention finit donc par bloquer toute nouvelle mise en file. C'est ce qui a gelé toutes les campagnes multi-`wait` après leur 2e envoi : voir `docs/incidents/2026-09-03-wait-node-resume-jobid-dedupe.md`. Règle : un `jobId` explicite doit avoir une durée de vie bornée et alignée sur la fenêtre de dédup voulue (ex. y intégrer `scheduled_at`), jamais « éternelle ». Le scheduler des `wait` (`campaign-engine.schedule_due_executions`) n'utilise volontairement **plus** de `jobId` — les `advance` en double sont déjà inoffensifs (lock optimiste + idempotence d'envoi).
