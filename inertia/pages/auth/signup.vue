  ...
  <div class="grid min-h-screen lg:grid-cols-2">
    ...
    <div class="flex flex-col px-6 pt-14 pb-10 sm:px-10 lg:justify-center lg:px-20 lg:py-16">
      <div class="mx-auto w-full max-w-sm auth-copy">
        ...
        <p class="tag-mono mb-3 text-[0.7rem] text-primary">GET STARTED</p>
        <h1 class="headline text-3xl">Create your account</h1>
        <p class="mt-2.5 mb-9 text-sm opacity-60">Set up your workspace in under a minute.</p>

        <Form v-slot="{ processing, errors }" route="new_account.store" class="flex flex-col gap-5">
          <label for="fullName" class="form-control block">
            <span class="field-label mb-1.5 block">Full name</span>
            <input
              id="fullName"
              type="text"
              name="fullName"
            ...
            />
            <span v-if="errors.fullName" class="mt-1.5 block text-sm text-error">{{
              errors.fullName
            }}</span>
          </label>
        ...
        <p class="mt-8 text-center text-sm opacity-60">
          ...
          <Link route="session.create" class="link link-primary font-medium no-underline"
            >Log in</Link
          >
        </p>
      </div>
